...
function BookingSection({ id,slug,price }: BookingSectionProps) {
  ...
  const [checAvailability, {isLoading}] = useCheckAvailabilityMutation();
  ...
  const handleBook = async() => {
    ...
    try {
      const data = {
        listing_id: id,
        start_date: moment(startDate).format("YYY-MM-DD"),
        end_date: moment(endDate).format("YYY-MM-DD"),
      }
      const res = await checAvailability(data).unwrap();
      if (res.success) {
